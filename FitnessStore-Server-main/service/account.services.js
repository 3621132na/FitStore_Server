  ...
  try {
    ...
    if (req.file) {
      ...
      Account.findByPk(newAccount.id)
        .then((instance) => {
          if (instance) {
            instance.image = 'http://localhost/resource/accounts/' + newAccount.id + '/' + newAccount.id + '.jpg';
            return instance.save();
          } else {
            console.log('Không tìm thấy đối tượng để cập nhật.');
          }
  ...
  try {
    ...
    if (req.file) {
      ...
      Account.findByPk(account.id)
        .then((instance) => {
          if (instance) {
            instance.image = 'http://localhost/resource/accounts/' + account.id + '/' + account.id + '.jpg';
            return instance.save();
          } else {
            console.log('Không tìm thấy đối tượng để cập nhật.');
          }
